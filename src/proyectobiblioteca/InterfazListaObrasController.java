...
public class InterfazListaObrasController implements Initializable {
    ...
    @FXML private TextField textFieldBuscarTitulo;
    @FXML private TextField textFieldBuscarAutor;
    @FXML private Label labelNombreUsuario;
    @FXML private Label labelFechaPrestamo;
    @FXML private Label labelFechaRetorno;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        textFieldBuscarAutor.setPromptText("Buscar por autor...");
        textFieldBuscarTitulo.setPromptText("Buscar por título...");

        //Filtro para buscar en la tabla por nombre de autor
        textFieldBuscarAutor.textProperty().addListener((prop, old, text) -> {
        ...
        textFieldBuscarTitulo.textProperty().addListener((prop, old, text) -> {
            filteredData.setPredicate(obra -> {
                if (text == null || text.isEmpty()) {
                    return true;
                }
            ...
            row.setOnMouseClicked(event -> {
                ...
                for (int i = 0; i <= librosPrestados.size() - 1; i++) {
                    ...
                    if (tituloTabla.equals(tituloPrestamo)) {
                        labelNombreUsuario.setText(prestarLibro.getUsuario());
                        labelFechaPrestamo.setText(prestarLibro.getFechaPrestamo().toString());
                        labelFechaRetorno.setText(prestarLibro.getFechaRetorno().toString());
                    }
                }
            });
            return row;
